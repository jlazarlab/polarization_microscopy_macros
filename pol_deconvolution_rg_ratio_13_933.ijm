...
macro "Mixed polarization image processing [g]"{
...
macro "Single-polarization image processing [h]" {
	raw_image_id = getImageID();
	raw_image_name = getTitle();
	number_of_slices = nSlices;
	w=getWidth;
	h=getHeight;
	slice_number = getSliceNumber();
	getPixelSize(pixel_size_unit, pixel_width, pixel_height);

	file_name_base = replace(raw_image_name,".tif","");
	file_name_base = replace(raw_image_name,".oif","");
	file_name_base = replace(raw_image_name,".oib","");

	number_of_polarizations = 1;		
	

	read_params_from_config_file();   // reads parameters from a config file; modifies values of global vars

	//finds a rectangle (w/10, h/10) with the lowest brightness for background subtraction, sets its intensity as bkgd_intensity
	bkgd_intensity = find_bkgd_intensity(raw_image_id);


	check_with_user = true;
	while(check_with_user == true){
		main_h_dialog_values_text = get_main_h_dialog_values(raw_image_id, bkgd_intensity);  // modifies global vars, incl. check_with_user; returns all values as 'key1:value1\nkey2:value2\n...'
		main_h_dialog_values_rows = split(main_h_dialog_values_text,"\n");	
		for(i=0; i<main_h_dialog_values_rows.length; i++){
			key_value_pair = split(main_h_dialog_values_rows[i],":");
			if(key_value_pair[0] == "bkgd_intensity"){bkgd_intensity = parseInt(key_value_pair[1]);}				// this gets the bkgd_intensity; most other params are in global vars
			if(key_value_pair[0] == "bkgd_subbed_image_id"){bkgd_subbed_image_id = parseInt(key_value_pair[1]);}	
			if(key_value_pair[0] == "check_with_user"){check_with_user = key_value_pair[1];}						
			if(key_value_pair[0] == "look_for_vesicles"){look_for_vesicles = key_value_pair[1];}						
		}		
		if(check_with_user == true){
			selectImage(bkgd_subbed_image_id);
			close();
		}
	}
	...
	if(pixel_size_unit == "micron" || pixel_size_unit == "microns" || pixel_size_unit == "micrometer" || pixel_size_unit == "micrometer" || pixel_size_unit == "um" || pixel_size_unit == "nanometer" || pixel_size_unit == "nanometers" || pixel_size_unit == "nm" || pixel_size_unit == "meter" || pixel_size_unit == "meters" || pixel_size_unit == "m"){
		...
		cutoff_distance_from_snake_in_pixels = cutoff_distance_from_snake;
	}
	run("Line Width...", "line="+toString(cutoff_distance_from_snake_in_pixels)); 
	...
	selectImage(bkgd_subbed_image_id);
	if(polarization_direction == "horizontal"){pol_suffix = "_HOR.tif";}
	else{pol_suffix = "_VER.tif";}	
	rename(file_name_base+pol_suffix);
	...
	BIN_image_id = make_BIN_image(segmentation_image_id, bkgd_subbed_image_id, cutoff_distance_from_snake_in_pixels);	//the segmentation image has a selection representing the snake
	...
	// now I should have a _BIN file (segmentation_image_id), snake coordinates (snake_points_x, snake_points_y) and HOR/VER file (bkgd_subbed_image_id)
	// I will send the _BIN file (bearing a snake_points_x, snake_points_y selection) and a background-subtracted fluorescence image for processing

	//theta = newArray(snake_points_x.length);
	
	if(determine_rmax_alpha0_sigma == "1PPM: r(max)"){LD_quantitation_results = perform_LD_quantitation_1PPM(BIN_image_id, bkgd_subbed_image_id);}
	else{LD_quantitation_results = perform_LD_quantitation_2PPM(BIN_image_id, bkgd_subbed_image_id);}
	...
	if(save_files == false){
		...
		selectImage(hyperstack_image_id);
		if(determine_rmax_alpha0_sigma == "1PPM: r(max)"){saveAs(File.directory+File.separator+file_name_base+"_1P_GFIT.tif");}
		else{saveAs(File.directory+File.separator+file_name_base+"_2P_GFIT.tif");}
	}

	if(orig_image_close == true){
		selectImage(raw_image_id);
		close();
	}

	//path_to_segmented_file = segmentation_function(bkgd_subbed_image_id); //does segmentation, saves segmented file, returns path to the segmented file
...
macro "Combining 1PPM, 2PPM data [c]"{
...
macro "Fitting by two Gaussian distributions [t]"{
	setBatchMode(true);
...
macro "Accessory: Segmentation/spline fitting [s]"{
	quant_processing_image_ID = getImageID();

	path_to_segmented_file = segmentation_function(quant_processing_image_ID);
	// path_to_segmented_file = path to the _BIN.tif file, including the name of the _BIN.tif file

	print(path_to_segmented_file);
}	
...
macro "Accessory: LD quantitation (1PPM, 2PPM) [l]"{
	...
	File.openDialog("Open/select a HOR, VER, or BIN file");
	file_directory = File.directory;
	file_name_base = substring(File.name, 0, lengthOf(File.name)-8);


	BIN_file_name = file_name_base + "_BIN.tif";
	open(file_directory+BIN_file_name);
	BIN_image_id = getImageID();
	run("To ROI Manager");
	roiManager("Select", 0);
	
	HOR_file_name = file_name_base + "_HOR.tif";
	if(File.exists(file_directory+HOR_file_name)){
		open(file_directory+HOR_file_name);
		HOR_image_id = getImageID();
		HOR_VER_image_id = HOR_image_id;	
		hor_exists = true;
	}

	VER_file_name = file_name_base + "_VER.tif";
	if(File.exists(file_directory+VER_file_name)){
		open(file_directory+VER_file_name);
		VER_image_id = getImageID();
		HOR_VER_image_id = VER_image_id;	
		ver_exists = true;
	}

	if(hor_exists == true && ver_exists == true){
		selectImage(HOR_image_id);
		run("Duplicate...", "title="+file_name_base+"_HOR_VER.tif");
		HOR_VER_image_id = getImageID();
		run("Add Slice");
		selectImage(VER_image_id);
		run("Select All");
		run("Copy");
		selectImage(HOR_VER_image_id);
		setSlice(2);
		run("Paste");
				
		Dialog.create("Image processing parameters");
		if(look_for_pol_angles == 1){identical_phase = true;}
		else{identical_phase = false;}
		Dialog.setInsets(10, 10, 10);
		Dialog.addMessage("Enter/select parameters for processing an image\nacquired using two polarizations of excitation light:");
		Dialog.addCheckbox("Identical deviation from hor./ver. directions ", identical_phase);
		Dialog.addCheckbox("Set deviation from hor./ver. directions manually", phase_fixed);
		Dialog.setInsets(0, 0, 0);
		Dialog.addNumber("Hor. polarization deviation ["+degreechar+"]:", phase1_deg,2,6,"");
		Dialog.addNumber("Ver. polarization deviation ["+degreechar+"]:", phase2_deg,2,6,"");
		...
		identical_phase = Dialog.getCheckbox();	// true = 1 phase; false = 2 phases
		phase_fixed = Dialog.getCheckbox();			// true = set hor./ver. directions manually; false = look for phases
		phase1_deg = Dialog.getNumber();
		phase2_deg = Dialog.getNumber();
		if(phase_fixed == true){look_for_pol_angles = 0;}
		else{
			if(identical_phase == true){look_for_pol_angles = 1;}
			if(identical_phase == false){look_for_pol_angles = 2;}
		}
		...
		ratiometric_fitting = true;
	}
	else{  // only one polarization
		Dialog.create("Image processing parameters");

		Dialog.setInsets(10, 10, 10);
		Dialog.addMessage("Enter/select parameters for processing an image\nacquired using one polarization of excitation light:");
		
		Dialog.setInsets(5, 60, 5);
		Dialog.addRadioButtonGroup("Analysis type:", newArray("1PPM: r(max)", "2PPM: r(max), "+alpha0char+", "+sigmachar), 2, 1, determine_rmax_alpha0_sigma);
		ratiometric_fitting_choices = newArray("non-ratiometric", "ratiometric");
		Dialog.setInsets(5, 60, 5);
		Dialog.addRadioButtonGroup("Data fitting:", ratiometric_fitting_choices, 2, 1, ratiometric_fitting_choices[ratiometric_fitting]);
		Dialog.setInsets(15, 20, 0);
		Dialog.addCheckbox("Set deviation from hor./ver. directions manually", phase_fixed);
		Dialog.addNumber("Polarization deviation from hor./ver. direction ["+degreechar+"]:", phase1_deg,2,6,"");
		...
		determine_rmax_alpha0_sigma = Dialog.getRadioButton();
	...
	if(determine_rmax_alpha0_sigma == "1PPM: r(max)"){LD_quantitation_results = perform_LD_quantitation_1PPM(BIN_image_id, HOR_VER_image_id);}
	else{LD_quantitation_results = perform_LD_quantitation_2PPM(BIN_image_id, HOR_VER_image_id);}
	...
	if(save_files == true){
		...
		if(determine_rmax_alpha0_sigma == "1PPM: r(max)"){
			rename(file_name_base+"_1P_FIT");
			//saveAs(File.directory+file_system_separator+file_name_base+"_1P_FIT.png");
		}
		else{
			rename(file_name_base+"_2P_FIT");
			//saveAs(File.directory+file_system_separator+file_name_base+"_2P_FIT.png");
		}
		saveAs("png");
	
		file_name_base = replace(File.name, "_FIT.png+","");
			
		selectImage(theta_image_id);
		save(File.directory+File.separator+file_name_base+"_THT.tif");
		close();

		File.saveString(fitting_data_text, File.directory+File.separator+file_name_base+"_ALL.txt");
		File.saveString(fitting_results_text, File.directory+File.separator+file_name_base+"_FIT.txt");
	
		selectImage(hyperstack_image_id);
		rename(file_name_base+"_GFIT");
		saveAs(File.directory+File.separator+file_name_base+"_GFIT.tif");
...
macro "Accessory: Reload macro [r]" {
	run("Install...", getInfo("macro.filepath"));
}


macro "Accessory: Close all Windows [x]" {
...
//exit;
macro "Accessory: process manually acquired image composite [z]"{
...
macro "Copy a list of intensities [i]"{
	clipboard_text = "";
	...
	for (i = 1; i <= frames; i++) {
		...
		clipboard_text = clipboard_text + toString(mean) + "\n";
	}
	Stack.setPosition(channel, slice, frame);

	String.copy(clipboard_text);
/*
...
macro "Accessory: make new BIN file [y]"{
	bin_image_id = getImageID();
	bin_image_name = getInfo("image.filename");
	imageCalculator("Divide create 32-bit", bin_image_id, bin_image_id);
	selectImage(bin_image_id);
	close()
	print("\\Clear");
	snk = "31.1067,24.0821;31.0882,24.2236;...";
	coord_pairs = split(snk, ";");
	x = newArray(coord_pairs.length-1);
	y = newArray(coord_pairs.length-1);
	for (i = 0; i < coord_pairs.length-1; i++) {
		coord_pair = coord_pairs[i];
		coord_pair_array = split(coord_pair,",");
		x[i] = coord_pair_array[0];
		y[i] = coord_pair_array[1];
		print(x[i],y[i]);
	}
	toUnscaled(x, y);
